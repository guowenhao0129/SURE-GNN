...
class WeightedGraphConv(nn.Module):
    def __init__(self, in_feats, out_feats, edge_weight):
    ...
    def forward(self, graph):
        with graph.local_scope():
            ...
            feat = graph.ndata['h_new'].to('cuda')
            out = self.linear(feat)
            return out

class Spectral_Beta(nn.Module):
    def __init__(self, in_feats, h_feats, num_classes, graph, w_tanhscore, d=2):
        ...
        self.conv3 = WeightedEdgeConv(in_feats, h_feats, self.num_edges)
        subgraph_w_tanhscore = w_tanhscore[edge_ids].unsqueeze(1).to('cuda')
        self.weconv = WeightedGraphConv(in_feats, h_feats, subgraph_w_tanhscore)
        del subgraph_w_tanhscore
        self.relation_dict = {relation: self.g[relation] for relation in self.g.canonical_etypes if
                              relation != ('r', 'homo', 'r')}
